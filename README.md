...
| `/m2_pr_release_auto` | 無人職守：自動 commit 目前變更 → `/m2_pr auto` 合併 → `/m2_release auto` 發版 → 驗證 GitHub Release | 會 commit 已追蹤變更與本次新增的檔案 + 改版本號 |
...
- 只有前三支接受後置 `auto`；`/m2_pr_release_auto` 本身就是固定的全自動指令，不需再加參數。
