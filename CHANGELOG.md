...
- **`/m2_evolve` 更名為 `/m2_evo`**，檔案 `.github/prompts/m2_evolve.prompt.md` → `m2_evo.prompt.md`。
  - 依本檔開頭的版本規則，「prompt 檔更名、slash command 改名」屬 **major**。
  - **下游 repo 需注意**：舊的 `/m2_evolve` 在同步後**不再存在**，任何腳本、文件或習慣打法都要改成 `/m2_evo`。
  - 行為完全不變，只有指令名稱變短；子指令（`resume` / `status` / `checkpoint` / `pause` / `stop`）維持原樣。
  - 帳本目錄仍為 `.evolve/`（它描述的是「回合帳本」這個概念，不是指令名稱），
    `.git/info/exclude` 與 `evolve/*` 分支命名一併維持不變，避免既有進行中的回合失效。
  - 同步更新：`copilot-instructions.md` §9 routing 表與 Auto Mode 章節、README 命令表／流程說明／目錄樹、
    以及 prompt 內文所有自我引用（觸發清單、§6 子指令對照、§6.1 Resume）。

### 修正

- **補回被吃掉的 CHANGELOG 章節標題 `## v2.1.0` 與 `## v2.2.0`。**
  前兩次新增版本條目時，取代區塊誤把下一個版本的標題一起吃掉，導致 v2.1.0 與 v2.2.0 的內容
  被併進上一個版本區塊。內容本身未遺失，只是標題不見了。
